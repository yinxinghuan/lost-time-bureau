...
- 类型：响应式竖屏 DOM；390×844 为视觉基准，真实界面使用逐页 Grid，而非固定截图比例或全页缩放。
...
| Clue | 这条线索说明什么？ | 图示与 2 个事实 | BACK TO VISITOR | 关闭按钮 | 关闭按钮 |
...
- 不再把 C 方向当作纯配色皮肤。线索、判定反馈、地图和结算都必须保留 C 的独立网格结构。
- 线索固定为“标题栏 → 插画 → 双事实格 → 可伸缩解释区 → 底部返回”；只有解释区可吃掉多余高度。
- 判定固定为“结果横幅 → 人物/关键理由 → 选择对照 → 下一步”；详细理由才进入第二层。
- 地图固定为居中时间线，节点卡片交替落在左右两边；路线为主，卡片为辅。
- 结算固定为“顶部得分 → 居中结局意义 → 分数事实格 → 底部重开”；不允许让空白随机挤开任何一组信息。
